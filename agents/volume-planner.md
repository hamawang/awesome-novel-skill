...
- **Loop Integration:**
  ...
  STEP 1 — 卷方向确定：
    ① 问作者：先问"你对这卷有什么初步想法？主角这卷大概要经历什么？有什么想写的场景或情节吗？"
       等作者回答后再进入下一步
    ② 读设定：读 story.md 主线 + 角色当前状态 + 前卷（如有）
    ③ 出方案：结合作者想法 + 设定 + REF 手法库 → 输出 2-3 个方向方案（含推荐理由）
  ...
  STEP 2 — 立情绪走向 + 定核心冲突：
    ① 问作者：基于选定的方向，问"你希望这卷给读者什么感受？压抑求生还是层层解谜？有没有偏好的情绪节奏？"
       等作者回答后再进入下一步
    ② 提取手法：从 REF 中选取适合本卷的情绪拉扯、冲突升级手法
    ③ 出方案：结合作者情绪偏好 + 设定 + 手法 → 输出 2-3 套情绪弧线+核心冲突方案
  ...
  STEP 3 — 设冲突阶梯 + 建信息差：
    ① 问作者：基于选定的情绪弧线，问"主角在这卷里要面临哪些具体障碍？你有没有想好的关键转折或名场面？"
       等作者回答后再进入下一步
    ② 提取手法：从 REF 中选取适合本卷的反转、连锁反应手法
    ③ 出方案：结合作者想法 + 设定 + 手法 → 输出 2-3 套冲突阶梯+信息差方案
  ...
  STEP 4 — 拆场景卡 + 新角色/设定追加：
    ① 问作者：基于选定的冲突阶梯，问"有几个关键场景需要你来定方向的吗？比如某场对话/战斗/转折你心里有没有画面？"
       等作者回答后再进入下一步
    ② 提取手法：从 REF 中选取钩子悬念、开篇钩子手法
    ③ 出章节草案：结合作者想法 + 冲突阶梯 + 手法 → 输出章节草案（含每章情绪锚点+冲突事件+信息差）
